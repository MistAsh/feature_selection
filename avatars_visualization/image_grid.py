...
from avatars_visualization.encoding import (
    FEATURES_MAPPING,
    TOTAL_BITS_COUNT,
    MAX_LENGTH_ENCODING_LABEL
)
from avatars_visualization.encoding import feature_count, encode_feature, \
    get_mapping_values, convert_to_avatar, from_avatar_to_image
import matplotlib.patches as patches


def create_image_grid(images, max_size_fig=6):
    """
    Creates a grid of images and returns figure and axes.  Optimized for
    animation.
    """
    n_images = len(images)
    if n_images == 0:
    ...
    # Turn off all axes and store image objects
    image_objects = []
    for ax, img in zip(axes_flat, images):
        ax.axis('off')
        # Store the image object for later updating
        im = ax.imshow(img)
        image_objects.append(im)

    # Handle cases where there are fewer images than axes
...
def animate_grid(data, fps=1, gif=True, save_path=None, max_fig_size=6):
    """
    Animates a grid of images. Efficiently updates existing image objects.
    """

    # --- Create the figure and subplots *ONCE* outside the loop ---
    first_frame_images = data[0]['images']
    fig, axes, image_objects = create_image_grid(first_frame_images, max_fig_size)

    # --- Initialize the title with the *FIRST FRAME'S TITLE* ---
    title_text = fig.suptitle(data[0]['title'])  # Use the first frame's title!
    ...
    def update(frame):
        ...
        # Update existing image objects
        for i, img in enumerate(images):
            if i < len(image_objects):
                image_objects[i].set_data(img)

        # Update the title text
        title_text.set_text(d['title'])
        if title_text._text != '':  # Avoid empty titles
            title_text.set_bbox({"facecolor": "lightgray", "alpha": 0.5, "pad": 5})

        # Return *all* changed artists, including the title
        return image_objects + [title_text]

    anim = animation.FuncAnimation(
        fig, update, frames=len(data), blit=True, interval=10
    )
    writer = (
        animation.PillowWriter(fps=fps) if gif else animation.FFMpegWriter(fps=fps)
    )
...
def visualization(
    ...
    if vector.shape[0] < TOTAL_BITS_COUNT:
        vector = np.pad(vector, (0, TOTAL_BITS_COUNT - vector.shape[0]))
    ...
    plt.suptitle("Avatar Feature Encoding Visualization", fontsize=16, y=0.98)
    plt.figtext(0.5, 0.01,
                "This visualization shows how binary vectors map to avatar features",
